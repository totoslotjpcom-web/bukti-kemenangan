...
function makeBasic(id,password){
...
async function api(url,opt={}){
  ...
  if(r.status===401){
    adminAuth="";
    showLogin();
    throw new Error("ID atau password admin tidak valid.");
  }

  if(!r.ok){
    throw new Error(d.error||`Request gagal (${r.status})`);
  }
...
$("loginForm").onsubmit=async e=>{
  ...
  adminAuth=makeBasic(id,password);
...
function showTab(n){
  ["dashboard","posts","new"].forEach(
    x=>$("tab-"+x).classList.toggle("hidden",x!==n)
  );

  document.querySelectorAll("[data-tab]").forEach(
    b=>b.classList.toggle("active",b.dataset.tab===n)
  );

  if(n==="new"&&!$("postId").value){
    resetForm(false);
  }
}
...
async function refresh(){
  ...
  $("recent").innerHTML=p.slice(0,5).map(x=>`
    <div style="padding:11px 0;border-bottom:1px solid #282318">
      <b>${esc(x.title)}</b>
      <div class="meta">
        ${esc(x.username)} • ${money(x.amount)} • ${esc(x.status)}
      </div>
    </div>
  `).join("") || '<div style="color:#888">Belum ada posting.</div>';
  ...
  $("postsRows").innerHTML=p.map(x=>`
    <tr>
      ...
      <td>
        <button class="btn small secondary" onclick="editPost(${x.id})">
          Edit
        </button>
        <button class="btn small danger" onclick="deletePost(${x.id})">
          Hapus
        </button>
      </td>
    </tr>
...
async function upload(file,type){
  ...
  return await api("/api/admin/upload",{
    method:"POST",
    body:fd
  });
}
...
function previews(input,target){
  target.innerHTML=[...input.files].map(
    f=>`<img class="thumb" src="${URL.createObjectURL(f)}">`
  ).join("");
}
...
async function editPost(id){
  ...
  $("publishedAt").value=
    p.published_at ? p.published_at.slice(0,16) : "";
  ...
  $("bannerPreview").innerHTML=
    currentBanner
      ? `<img class="thumb" src="${esc(currentBanner)}">`
      : "";

  $("winPreview").innerHTML=
    currentMedia
      .filter(x=>x.media_type==="win_proof")
      .map(x=>`<img class="thumb" src="${esc(x.image_url)}">`)
      .join("");

  $("transferPreview").innerHTML=
    currentMedia
      .filter(x=>x.media_type==="transfer_proof")
      .map(x=>`<img class="thumb" src="${esc(x.image_url)}">`)
      .join("");
...
function resetForm(clear=true){
  ...
  ["bannerPreview","winPreview","transferPreview"].forEach(
    x=>$(x).innerHTML=""
  );
